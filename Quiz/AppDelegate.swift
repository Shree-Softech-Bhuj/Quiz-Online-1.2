...
//            NSLog("[RemoteNotification] didFinishLaunchingWithOptions : \(userInfo)")
...
//        if (remoteMessage.appData["title"] ) != nil {
//           title = remoteMessage.appData["title"]  as! String
//        }
//        if (remoteMessage.appData["body"] ) != nil {
//           body = remoteMessage.appData["body"]  as! String
...
//               }
        if (remoteMessage.appData["notification"] as? [String:Any]) != nil { //cloud message - firebase
            ...
            if DATA["image"] != nil {
              ...
              //  let urlImg: URL = URL(fileURLWithPath: img)
            ...
            fragmentTestData(str) //just for testing @ https://demo.androidhive.info/firebase/notifications/
            if  isImgAttached == true {
                DispatchQueue.main.asyncAfter(deadline: .now() + 7.0, execute: {
                    self.showNotificationWithAttachment(self.title,self.body,self.imgURL!) //showNotification(title,body,img)
                })
            ...
            //from androidhive testing
//            remoteMessage.appData.contains { (data) -> Bool in
//
//                let (key, value) = data
//                 print("\(key) - \(value)")
//                fragmentTestData(value as! String)
//
//                    if  isImgAttached == true {
//                       DispatchQueue.main.asyncAfter(deadline: .now() + 7.0, execute: {
//                           self.showNotificationWithAttachment(self.title,self.body,self.imgURL!) //showNotification(title,body,img)
...
//            }
            ...
            //else{
//              if(message.containsKey('datafrom'))
//              datafrom = message["datafrom"];
//              if(message.containsKey('nameFrom'))
//              namefrom = message["nameFrom"];
//              if(message.containsKey('idFrom'))
...
//        })
    ...
    func fragmentTestData(_  str:String){
    ...
    func removeTempImg(){ //remove Img before copying image downloaded from url given with notification data
       let  documentsUrl:URL =  FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let destinationFileUrl = documentsUrl.appendingPathComponent("tempImg.jpg")

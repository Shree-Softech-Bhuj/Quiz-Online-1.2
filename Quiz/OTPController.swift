...
class OTPController: UIViewController ,UITextFieldDelegate{
    ...
    @IBOutlet weak var codeTxt: FloatingTF!    
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        scroll.setContentOffset(CGPoint(x: 0,y: textField.center.y-400), animated: true)
    }
    ...
    @IBAction func verifyBtn(_ sender: UIButton) {
      //check if codetxt is not empty
        ...
        if  self.codeTxt.text!.trimmingCharacters(in: .whitespacesAndNewlines) == ""
        {
            ...
            print(" get veri. ID \(verificationID!)")//(String(describing: verificationID))
            ...
            Auth.auth().signIn(with: credential) { (authResult, error) in
              //  print("error -- \(error!) --- authResult \(String(describing: authResult))")
              if let error = error {
            ...
            //once verification done , login user details
                ...
                let fid = authResult?.user.uid //firebase id
        ...
        //        // send data to server after successfully logged in
                let apiURL = "name=\(self.name)&email=&profile=&mobile=\(num)&type=mobile&fcm_id=\(Apps.FCM_ID)&ip_address=1.0.0&status=0&friends_code=\(self.frndCode)&refer_code=\(Apps.REFER_CODE)&firebase_id=\(fid!)"//&firebase_id=\(authResult?.user.uid)"
                print(apiURL)

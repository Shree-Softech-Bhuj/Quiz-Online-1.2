...
//        }
        ...
        for itemCount in 0 ..< numberOfItems {
            ...
            let indexInGroup = itemCount % 3
            let indexPath = IndexPath(item: itemCount, section: SECTION)
            print("indexInGroup -- \(indexInGroup) - indexpath -- \(indexPath)")
            guard let attributes = cellAttributes.object(forKey: indexPath) as? UICollectionViewLayoutAttributes else {
                return
            }
            ...
            if self.currentOrientation.isPortrait {
                ...
                case 2:
                    ...
                    break
               case 0:
                    frame = CGRect(x: x, y: y, width: internalGridCellSize.width * 2 + self.gridCellSpacing.width, height: internalGridCellSize.height  + self.gridCellSpacing.height) //* 2
                    y += frame.size.height + self.gridCellSpacing.height
                    print("size case 0")
                    break
                default:
